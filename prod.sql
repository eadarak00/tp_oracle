...
CREATE OR REPLACE PROCEDURE Afficher_Resultats_Classe(niveau IN CHAR, semestre IN CHAR, annee IN CHAR)
AS
    cycle Semestre.Cycle%TYPE;
BEGIN
    SELECT Cycle INTO cycle FROM Semestre WHERE Numero = semestre AND Annee = annee;
    
    DBMS_OUTPUT.PUT_LINE('                          RÉSULTATS DE LA CLASSE                               ');
    DBMS_OUTPUT.PUT_LINE('Année             : ' || annee);
    DBMS_OUTPUT.PUT_LINE('Niveau            : ' || cycle || ' ' || semestre);
    DBMS_OUTPUT.PUT_LINE('Semestre          : Semestre ' || semestre);
    DBMS_OUTPUT.PUT_LINE('------------------------------------------------------------------------------');
    
    FOR rec IN (
        SELECT Resultat.*
        FROM Resultat, Etudiant
        WHERE Resultat.Etudiant = Etudiant.Matricule
            AND Resultat.Semestre = semestre
            AND Resultat.Annee = annee
            AND Etudiant.Niveau = niveau
    ) LOOP
...
CREATE OR REPLACE PROCEDURE Afficher_Classement_Etudiants(niveau IN CHAR, semestre IN CHAR, annee IN CHAR)
AS
    cycle Semestre.Cycle%TYPE;
BEGIN
    SELECT Cycle INTO cycle FROM Semestre WHERE Numero = semestre AND Annee = annee;
    ...
    DBMS_OUTPUT.PUT_LINE('==============================================================================');
    DBMS_OUTPUT.PUT_LINE('                 CLASSEMENT DES ÉTUDIANTS PAR MÉRITE                           ');
    DBMS_OUTPUT.PUT_LINE('==============================================================================');
    DBMS_OUTPUT.PUT_LINE('Année    : ' || annee);
    DBMS_OUTPUT.PUT_LINE('Niveau   : ' || cycle || ' ' || semestre);
    DBMS_OUTPUT.PUT_LINE('Semestre : Semestre ' || semestre);
...
CREATE OR REPLACE PROCEDURE Afficher_Releve_Etudiant(    
    num_carte IN CHAR, 
    niveau IN CHAR, 
    semestre IN CHAR, 
    annee IN CHAR)
...
EXCEPTION
    ...
    WHEN result_not_found THEN
        DBMS_OUTPUT.PUT_LINE('Erreur : Aucun résultat trouvé pour cet étudiant au semestre ' || semestre || ' de l''année ' || annee);
END;
/
...
CREATE OR REPLACE PROCEDURE Lister_Etudiants_Admis(
    niveau IN CHAR, 
    semestre IN CHAR, 
    annee IN CHAR
)
AS
BEGIN
    DBMS_OUTPUT.PUT_LINE('                  LISTE DES ÉTUDIANTS ADMIS                           ');
    DBMS_OUTPUT.PUT_LINE('======================================================================');
    ...
    FOR rec IN (
        ...
        ORDER BY Resultat.Moyenne DESC
    ) LOOP
        DBMS_OUTPUT.PUT_LINE(
...
CREATE OR REPLACE PROCEDURE Afficher_Majorant_Classe(
    niveau IN CHAR, 
    semestre IN CHAR, 
    annee IN CHAR
)
AS
    ...
    moy_etu     Resultat.Moyenne%TYPE;
    
    student_not_found EXCEPTION;
BEGIN
    DBMS_OUTPUT.PUT_LINE('======================================================================');
    DBMS_OUTPUT.PUT_LINE('                      MAJORANT DE LA CLASSE                           ');
    DBMS_OUTPUT.PUT_LINE('======================================================================');
    
    BEGIN
        SELECT Etudiant.Matricule, Etudiant.Nom, Etudiant.Prenom, Etudiant.Age, Etudiant.Sexe, Resultat.Moyenne
        INTO mat_etu, nom_etu, prenom_etu, age_etu, sexe_etu, moy_etu
        FROM Etudiant 
        JOIN Resultat ON Etudiant.Matricule = Resultat.Etudiant
        WHERE Etudiant.Niveau = niveau 
          AND Resultat.Semestre = semestre 
    ...
    DBMS_OUTPUT.PUT_LINE(
        ...
        'Âge       : ' || age_etu || '\n' ||
        'Sexe      : ' || sexe_etu || '\n' ||
        'Moyenne   : ' || TO_CHAR(moy_etu, '99.99')
    );
    
EXCEPTION
    WHEN student_not_found THEN
        DBMS_OUTPUT.PUT_LINE('Aucun majorant trouvé pour ce semestre et cette classe.');
END;
/
